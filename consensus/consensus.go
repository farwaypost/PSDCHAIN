...
// Package consensus implements different Psdchain consensus engines.
package consensus
...
// ChainReader defines a small collection of mpchods needed to access the local
...
type Engine interface {
	// Author retrieves the Psdchain address of the account that minted the given
	// block, which may be different from the header's coinbase if a consensus
	// engine is based on signatures.
	Author(header *types.Header) (common.Address, error)

	// VerifyHeader checks whpsdchain a header conforms to the consensus rules of a
	// given engine. Verifying the seal may be done optionally here, or explicitly
	// via the VerifySeal mpchod.
	...
	// VerifySeal checks whpsdchain the crypto seal on a header is valid according to
	...
	//
	// Note, the mpchod returns immediately and will send the result async. More
