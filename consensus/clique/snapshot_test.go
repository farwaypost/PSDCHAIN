...
// mapped from textual names used in the tests below to actual Psdchain private
...
// address retrieves the Psdchain address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Psdchain address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/psdchaineum/go-psdchaineum/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Gpch the header and prepare it for signing
			header := block.Header()
			if j > 0 {
