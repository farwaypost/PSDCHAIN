...
// Tests whpsdchain remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom pchash engine
	pchash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer pchash.Close()
	...
	pchash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := pchash.SealHash(header).Hex(); work[0] != want {
...
// Tests whpsdchain stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	pchash := NewTester(nil, true)
	defer pchash.Close()
	api := &API{pchash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			pchash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, pchash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch pchash result timeout", id+1)
		}
	}
}
