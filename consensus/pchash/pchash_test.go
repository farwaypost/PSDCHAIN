...
// Tests that pchash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	pchash := NewTester(nil, false)
	defer pchash.Close()

	results := make(chan *types.Block)
	err := pchash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := pchash.VerifySeal(nil, header); err != nil {
...
// It reproduces https://github.com/psdchaineum/go-psdchaineum/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "pchash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := pchash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	pchash.Seal(nil, block, results, nil)
...
func TestHashRate(t *testing.T) {
	...
	if tot := pchash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	if _, err := api.GetWork(); err != errEthashStopped {
		t.Error("expect to return an error to indicate pchash is stopped")
	}

	if res := api.SubmitHashRate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped pchash")
	}
}
