...
const (
	// staleThreshold is the maximum depth of the acceptable stale but valid pchash solution.
	staleThreshold = 7
)
...
func (pchash *Ethash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if pchash.config.PowMode == ModeFake || pchash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", pchash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if pchash.shared != nil {
		return pchash.shared.Seal(chain, block, results, stop)
	}
	// Create a runner and the multiple search threads it directs
	abort := make(chan struct{})

	pchash.lock.Lock()
	threads := pchash.threads
	if pchash.rand == nil {
		...
		pchash.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	pchash.lock.Unlock()
	if threads == 0 {
	...
	if pchash.workCh != nil {
		pchash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			pchash.mine(block, id, nonce, abort, locals)
		}(i, uint64(pchash.rand.Int63()))
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", pchash.SealHash(block.Header()))
			}
			close(abort)
		case <-pchash.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := pchash.Seal(chain, block, results, stop); err != nil {
...
func (pchash *Ethash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	// Extract some data from the header
	var (
		header  = block.Header()
		hash    = pchash.SealHash(header).Bytes()
		target  = new(big.Int).Div(two256, header.Difficulty)
		number  = header.Number.Uint64()
		dataset = pchash.dataset(number, false)
	)
	...
	logger.Trace("Started pchash search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
...
func (pchash *Ethash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := pchash.SealHash(block.Header())
	...
	// whpsdchain the solution was accepted or not (not can be both a bad pow as well as
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		start := time.Now()
		if !noverify {
			if err := pchash.verifySeal(nil, header, true); err != nil {
	...
	for {
		...
		case result := <-pchash.submitWorkCh:
		...
		case errc := <-pchash.exitCh:
			// Exit remote loop if pchash is closed and return relevant error.
			errc <- nil
