...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of pchash, we're pretty much done
...
// importGenesis imports a Gpch genesis spec into pupppch.
...
func (w *wizard) manageGenesis() {
	// Figure out whpsdchain to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alpch.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by pupppch and Gpch
		gpchJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gpchJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gpchJson)

		// Export the genesis spec used by Alpch (formerly C++ Psdchain)
		if spec, err := newAlpchGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Alpch chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alpch", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly PsdchainJ
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not sompching I can do")
		return
	}
}
