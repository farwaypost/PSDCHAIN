...
// pchstatsDockerfile is the Dockerfile required to build an pchstats backend
// and associated monitoring site.
var pchstatsDockerfile = `
FROM pupppch/pchstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// pchstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an pchstats monitoring site.
var pchstatsComposefile = `
version: '2'
services:
  pchstats:
    build: .
    image: {{.Network}}/pchstats
    container_name: {{.Network}}_pchstats_1{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new pchstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(pchstatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(pchstatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the pchstats service
	if nocache {
...
// pchstatsInfos is returned from an pchstats status check to allow reporting
// various configuration parameters.
type pchstatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *pchstatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an pchstats server to verify whpsdchain
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*pchstatsInfos, error) {
	// Inspect a possible pchstats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_pchstats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
