...
FROM pupppch/explorer:latest

ADD pchstats.json /pchstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../pch-net-intelligence-api && pm2 start /pchstats.json)' >  explorer.sh && \
	echo '(cd ../psdchainchain-light && npm start &)'                      >> explorer.sh && \
	echo 'exec /parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEthstats is the configuration file for the pchstats javascript client.
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.psdchaineum
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	pchstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEthstats)).Execute(pchstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.pchstats[:strings.Index(config.pchstats, ":")],
		"Secret": config.pchstats[strings.Index(config.pchstats, ":")+1 : strings.Index(config.pchstats, "@")],
		"Host":   config.pchstats[strings.Index(config.pchstats, "@")+1:],
	})
	files[filepath.Join(workdir, "pchstats.json")] = pchstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.psdchaineum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		pchstats: infos.envvars["STATS"],
	}
	return stats, nil
}
