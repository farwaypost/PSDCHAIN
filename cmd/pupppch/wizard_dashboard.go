...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"pchstats", "explorer", "wallet", "faucet"} {
	...
	// If we have pchstats running, ask whpsdchain to make the secret public or not
	if w.conf.pchstats != "" {
		fmt.Println()
		fmt.Println("Include pchstats secret on dashboard (y/n)? (default = yes)")
