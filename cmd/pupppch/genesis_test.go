...
// Tests the go-psdchaineum to Alpch chainspec conversion for the Stureby testnet.
func TestAlpchSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gpch.json")
	if err != nil {
	...
	spec, err := newAlpchGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alpch.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alpchGenesisSpec{}
...
// Tests the go-psdchaineum to Parity chainspec conversion for the Stureby testnet.
