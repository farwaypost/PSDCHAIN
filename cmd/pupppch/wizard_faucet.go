...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Psdchains to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.pchstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.pchstats = w.readString() + ":" + w.conf.pchstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.pchstats)
		infos.node.pchstats = w.readDefaultString(infos.node.pchstats) + ":" + w.conf.pchstats
	}
