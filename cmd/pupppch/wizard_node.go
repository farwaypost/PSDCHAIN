...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.pchstats == "" {
		log.Error("No pchstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.pchashdir == "" {
			fmt.Printf("Where should the pchash mining DAGs be stored on the remote machine?\n")
			infos.pchashdir = w.readString()
		} else {
			fmt.Printf("Where should the pchash mining DAGs be stored on the remote machine? (default = %s)\n", infos.pchashdir)
			infos.pchashdir = w.readDefaultString(infos.pchashdir)
		}
	}
	...
	if infos.pchstats == "" {
		...
		infos.pchstats = w.readString() + ":" + w.conf.pchstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.pchstats)
		infos.pchstats = w.readDefaultString(infos.pchstats) + ":" + w.conf.pchstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an psdchainbase to mine against
			fmt.Println()
			if infos.psdchainbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.psdchainbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.psdchainbase)
				infos.psdchainbase = w.readDefaultAddress(common.HexToAddress(infos.psdchainbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Psdchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
