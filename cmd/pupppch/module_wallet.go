...
FROM pupppch/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gpch --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gpch --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --pchstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PupppchNetworkID/{{.NetworkID}}/g' dist/js/psdchainwallet-master.js && \
	sed -i 's/PupppchNetwork/{{.Network}}/g'     dist/js/psdchainwallet-master.js && \
	sed -i 's/PupppchDenom/{{.Denom}}/g'         dist/js/psdchainwallet-master.js && \
	sed -i 's/PupppchHost/{{.Host}}/g'           dist/js/psdchainwallet-master.js && \
	sed -i 's/PupppchRPCPort/{{.RPCPort}}/g'     dist/js/psdchainwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whpsdchain
// it's running, and if yes, whpsdchain it's responsive.
