...
// makeWizard creates and returns a new pupppch wizard.
...
// setting up a new or managing an existing Psdchain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to pupppch, your Psdchain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Psdchain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and pchstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Pupppch uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Psdchain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".pupppch", w.network)
