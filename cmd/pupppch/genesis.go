...
import (
	...
	math2 "github.com/psdchaineum/go-psdchaineum/common/math"
...
// alpchGenesisSpec represents the genesis specification format used by the
// C++ Psdchain implementation.
type alpchGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alpchGenesisSpecAccount `json:"accounts"`
}

// alpchGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alpchGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alpchGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alpchGenesisSpecBuiltin is the precompiled contract definition.
type alpchGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock hexutil.Uint64                 `json:"startingBlock,omitempty"`
	Linear        *alpchGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alpchGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlpchGenesisSpec converts a go-psdchaineum genesis block into a Alpch-specific
// chain specification format.
func newAlpchGenesisSpec(network string, genesis *core.Genesis) (*alpchGenesisSpec, error) {
	// Only pchash is currently supported between go-psdchaineum and alpch
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Alpch format
	spec := &alpchGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	spec.Params.BlockReward = (*hexutil.Big)(pchash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &alpchGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alpchGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alpchGenesisSpecBuiltin{Name: "sha256",
		Linear: &alpchGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alpchGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alpchGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alpchGenesisSpecBuiltin{Name: "identity",
		Linear: &alpchGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alpchGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
		spec.setPrecompile(6, &alpchGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &alpchGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alpchGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &alpchGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alpchGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
	}
	return spec, nil
}

func (spec *alpchGenesisSpec) setPrecompile(address byte, data *alpchGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alpchGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alpchGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alpchGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alpchGenesisSpecAccount)
	}
...
func (spec *alpchGenesisSpec) setByzantium(num *big.Int) {
	spec.Params.ByzantiumForkBlock = hexutil.Uint64(num.Uint64())
}

func (spec *alpchGenesisSpec) setConstantinople(num *big.Int) {
...
// newParityChainSpec converts a go-psdchaineum genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only pchash is currently supported between go-psdchaineum and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// gpch has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Psdchain.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Psdchain.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Psdchain.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(pchash.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(pchash.ConstantinopleBlockReward)
...
// pyPsdchainGenesisSpec represents the genesis specification format used by the
// Python Psdchain implementation.
type pyPsdchainGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyPsdchainGenesisSpec converts a go-psdchaineum genesis block into a Parity specific
// chain specification format.
func newPyPsdchainGenesisSpec(network string, genesis *core.Genesis) (*pyPsdchainGenesisSpec, error) {
	// Only pchash is currently supported between go-psdchaineum and pypsdchaineum
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyPsdchainGenesisSpec{
