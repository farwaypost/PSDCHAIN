...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/signer/rules"
...
func init() {
	app.Name = "Clef"
	app.Usage = "Manage Psdchain account operations"
	app.Flags = []cli.Flag{
		logLevelFlag,
		keystoreFlag,
		configdirFlag,
...
func testExternalUI(api *core.SignerAPI) {
	...
	checkErr := func(mpchod string, err error) {
		if err != nil && err != core.ErrRequestDenied {
			errs = append(errs, fmt.Sprintf("%v: %v", mpchod, err.Error()))
		}
	}
	var err error
...
curl -H "Content-Type: application/json" -X POST --data '{"jsonrpc":"2.0","mpchod":"account_new","params":["test"],"id":67}' localhost:8550

// List accounts

curl -i -H "Content-Type: application/json" -X POST --data '{"jsonrpc":"2.0","mpchod":"account_list","params":[""],"id":67}' http://localhost:8550/
...
curl -i -H "Content-Type: application/json" -X POST --data '{"jsonrpc":"2.0","mpchod":"account_sign","params":["0x694267f14675d7e1b9494fd8d72fefe1755710fa","bazonk gaz baz"],"id":67}' http://localhost:8550/
