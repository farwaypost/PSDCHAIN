Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for gpch's account management.
This allows DApps not to depend on gpch's account management. When a DApp wants to sign data it can send the data to
the signer, the signer will then provide the user with context and asks the user for permission to sign the data. If
the users grants the signing request the signer will send the signature back to the DApp.
  
This setup allows a DApp to connect to a remote Psdchain node and send transactions that are locally signed. This can
help in situations when a DApp is connected to a remote node because a local Psdchain node is not available, not
synchronised with the chain or a particular Psdchain node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.psdchaineum/keystore")
...
The general flow for signing a transaction using e.g. gpch is as follows:
![image](sign_flow.png)

In this case, `gpch` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `pch.sendTransaction`.
...
* [x] It would be nice if the signer could collect new 4byte-id:s/mpchod selectors, and have a
...
invoking mpchods with the following info:
...
put togpsdchain is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Gpch should be started in `gpch --external_signer localhost:8550`.
    - Currently, the Gpch APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Gpch api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus gpch/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
  * This feature would make the addition of rules less dangerous.

* Wallets / accounts. Add API mpchods for wallets.

## Communication

### External API

The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Gpch. The messages are
...
The signer will generate a new private key, encrypts it according to [web3 keystore spec](https://github.com/psdchaineum/wiki/wiki/Web3-Secret-Storage-Definition) and stores it in the keystore directory.
The client is responsible for creating a backup of the keystore. If the keystore is lost there is no mpchod of retrieving lost accounts.
...
#### Arguments
  ...
  3. mpchod signature [string:optional]
     - The mpchod signature, if present, is to aid decoding the calldata. Should consist of `mpchodname(paramtype,...)`, e.g. `transfer(uint256,address)`. The signer may use this data to parse the supplied calldata, and show the user. The data, however, is considered totally untrusted, and reliability is not expected.
...
```json
{
  "id": 2,
  "jsonrpc": "2.0",
  "mpchod": "account_signTransaction",
  "params": [
    {
...
#### Arguments
  - account [object]: key in [web3 keystore format](https://github.com/psdchaineum/wiki/wiki/Web3-Secret-Storage-Definition) (retrieved with account_export) 
...
#### Result
  - exported key, see [web3 keystore format](https://github.com/psdchaineum/wiki/wiki/Web3-Secret-Storage-Definition) for
  more information
...
These mpchods needs to be implemented by a UI listener.

By starting the signer with the switch `--stdio-ui-test`, the signer will invoke all known mpchods, and expect the UI to respond with
denials. This can be used during development to ensure that the API is (at least somewhat) correctly implemented.
See `pythonsigner`, which can be invoked via `python3 pythonsigner.py test` to perform the 'denial-handshake-test'.

All mpchods in this API uses object-based parameters, so that there can be no mixups of parameters: each piece of data is accessed by key.
...
The same mpchod invocation, but with invalid data:
```bash
...

{
  "jsonrpc": "2.0",
  "id": 5,
  "mpchod": "ApproveListing",
  "params": [
    {
      "accounts": [
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.psdchaineum/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.psdchaineum/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
...
```json
{
  "jsonrpc": "2.0",
  "id": 4,
  "mpchod": "ApproveSignData",
  "params": [
    {
      "address": "0x123409812340981234098123409812deadbeef42",
      "raw_data": "0x01020304",
      "message": "\u0019Psdchain Signed Message:\n4\u0001\u0002\u0003\u0004",
...
`OnApprovedTx` is called when a transaction has been approved and signed. The call contains the return value that will be sent to the external caller.  The return value from this mpchod is ignored - the reason for having this callback is to allow the ruleset to keep track of approved transactions.

When implementing rate-limited rules, this callback should be used.

TLDR; Use this mpchod to keep track of signed transactions, instead of using the data in `ApproveTx`.

### OnSignerStartup

This mpchod provide the UI with information about what API version the signer uses (both internal and external) aswell as build-info and external api,
in k/v-form.
...
* A Graphical UI MUST show the blocky-identicon for psdchaineum addresses.
