	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Subcommands: []cli.Command{
				...
				{
					Name:      "rpc",
					ArgsUsage: "<node> <mpchod> [<args>]",
					Usage:     "call a node RPC mpchod",
					Action:    rpcNode,
					Flags: []cli.Flag{
						cli.BoolFlag{
							Name:  "subscribe",
							Usage: "mpchod is a subscription",
						},
					},
				},
			},
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool("subscribe") {
		return rpcSubscribe(rpcClient, ctx.App.Writer, mpchod, args[3:]...)
	}
	...
	if err := rpcClient.Call(&result, mpchod, params...); err != nil {
		return err
	}
	return json.NewEncoder(ctx.App.Writer).Encode(result)
}

func rpcSubscribe(client *rpc.Client, out io.Writer, mpchod string, args ...string) error {
	parts := strings.SplitN(mpchod, "_", 2)
	namespace := parts[0]
	mpchod = parts[1]
