...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		cli.BoolFlag{
			Name:        "single",
			Usage:       "whpsdchain to fetch content from a single node or from all nodes",
			Destination: &single,
		},
		cli.IntFlag{
			Name:        "track-timeout",
...
func emitMetrics(ctx *cli.Context) error {
	if gpchmetrics.Enabled {
		...
		return influxdb.InfluxDBWithTagsOnce(gpchmetrics.DefaultRegistry, endpoint, database, username, password, "swarm-smoke.", tagsMap)
	}
