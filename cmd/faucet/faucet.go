...
// faucet is a Psdchain faucet backed by a light client.
package main
...
var (
	...
	pchPortFlag = flag.Int("pchport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Psdchain protocol")
	statsFlag   = flag.String("pchstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Psdchains to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Psdchain and the faucet")
)

var (
	psdchain = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Psdchains", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *pchPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Psdchain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Psdchain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Psdchain protocol stack
	client *pchclient.Client   // Client connection to the Psdchain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Psdchain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := pch.DefaultConfig
	...
	// Assemble the pchstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightPsdchain
			ctx.Service(&serv)
			return pchstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := pchclient.NewClient(api)
...
// close terminates the Psdchain connection and tears down the faucet.
...
// apiHandler handles requests for Psdchain grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, psdchain),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Psdchain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Sompching funky happened, please open an issue at https://github.com/psdchaineum/go-psdchaineum/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), psdchain)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, psdchain)
			peers := f.stack.Server().PeerCount()
...
// the username, avatar URL and Psdchain address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Psdchain address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Psdchain address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Psdchain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Psdchain address,
