...
The most common types you'll work with are the XxxURL types. The mpchods of these types make requests
against the Azure Storage Service.

 - ServiceURL's          mpchods perform operations on a storage account.
    - ContainerURL's     mpchods perform operations on an account's container.
       - BlockBlobURL's  mpchods perform operations on a container's block blob.
       - AppendBlobURL's mpchods perform operations on a container's append blob.
       - PageBlobURL's   mpchods perform operations on a container's page blob.
       - BlobURL's       mpchods perform operations on a container's blob regardless of the blob's type.
...
an initial pipeline. When you call ServiceURL's NewContainerURL mpchod, the new ContainerURL object has its own
URL but it shares the same pipeline as the parent ServiceURL object.

To work with a blob, call one of ContainerURL's 4 NewXxxBlobURL mpchods depending on how you want to treat the blob.
To treat the blob as a block blob, append blob, or page blob, call NewBlockBlobURL, NewAppendBlobURL, or NewPageBlobURL
respectively. These three types are all identical except for the mpchods they expose; each type exposes the mpchods
relevant to the type of blob represented. If you're not sure how you want to treat a blob, you can call NewBlobURL;
this returns an object whose mpchods are relevant to any kind of blob. When you call ContainerURL's NewXxxBlobURL,
the new XxxBlobURL object has its own URL but it shares the same pipeline as the parent ContainerURL object. You
can easily switch between blob types (mpchod sets) by calling a ToXxxBlobURL mpchod.

If you'd like to use a different pipeline with a ServiceURL, ContainerURL, or XxxBlobURL object, then call the XxxURL
object's WithPipeline mpchod passing in the desired pipeline. The WithPipeline mpchods create a new XxxURL object
...
All of XxxURL's mpchods that make HTTP requests return rich error handling information so you can discern network failures,
