...
- Reintroduced NewxxClientWithBaseURI mpchod.
...
- Removed Api version from mpchod arguments.
- Removed New...ClientWithBaseURI() mpchod in all clients. BaseURI value is set in client.go.
...
- storage: Allow caller to provide headers for DeleteBlob mpchods.
...
- Changed mpchods to return `error` instead of `autorest.Error`.
- Re-divided mpchods to ease asynchronous requests.
