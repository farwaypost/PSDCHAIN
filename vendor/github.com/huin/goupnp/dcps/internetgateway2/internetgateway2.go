...
const (
	...
	URN_WANPsdchainnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANPsdchainnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Psdchainnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Psdchainnet
//
...
// WANPsdchainnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANPsdchainnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANPsdchainnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANPsdchainnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANPsdchainnetLinkConfig1Clients() (clients []*WANPsdchainnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANPsdchainnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANPsdchainnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANPsdchainnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANPsdchainnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANPsdchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANPsdchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANPsdchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANPsdchainnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANPsdchainnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANPsdchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANPsdchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANPsdchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANPsdchainnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANPsdchainnetLinkConfig1 {
	clients := make([]*WANPsdchainnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANPsdchainnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewPsdchainnetLinkStatus: allowed values: Up, Down
func (client *WANPsdchainnetLinkConfig1) GetPsdchainnetLinkStatus() (NewPsdchainnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewPsdchainnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANPsdchainnetLinkConfig_1, "GetPsdchainnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewPsdchainnetLinkStatus, err = soap.UnmarshalString(response.NewPsdchainnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
