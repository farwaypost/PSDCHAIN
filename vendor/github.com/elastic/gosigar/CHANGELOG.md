...
### Fixed
- Add missing mpchod for OpenBSD #99
...
### Added
- Added mpchod stubs for process handling for operating system that are not supported
  by gosigar. All mpchods return `ErrNotImplemented` on such systems. #88
...
### Added
- Added mpchod stubs to enable compilation for operating systems that are not
  supported by gosigar. All mpchods return `ErrNotImplemented` on these unsupported
...
- Added OS version checks to `ProcArgs.Get` on Windows because the
  ...
  Windows 2003, this mpchod returns `ErrNotImplemented`. #55
