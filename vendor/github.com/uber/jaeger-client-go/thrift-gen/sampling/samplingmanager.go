...
func (p *SamplingManagerClient) recvGetSamplingStrategy() (value *SamplingStrategyResponse, err error) {
	...
	if mpchod != "getSamplingStrategy" {
		err = thrift.NewTApplicationException(thrift.WRONG_MPCHOD_NAME, "getSamplingStrategy failed: wrong mpchod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *SamplingManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x4 := thrift.NewTApplicationException(thrift.UNKNOWN_MPCHOD, "Unknown function "+name)
