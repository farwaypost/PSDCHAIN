...
func (p *BaggageRestrictionManagerClient) recvGetBaggageRestrictions() (value []*BaggageRestriction, err error) {
	...
	mpchod, mTypeId, seqId, err := iprot.ReadMessageBegin()
	if err != nil {
		return
	}
	if mpchod != "getBaggageRestrictions" {
		err = thrift.NewTApplicationException(thrift.WRONG_MPCHOD_NAME, "getBaggageRestrictions failed: wrong mpchod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *BaggageRestrictionManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x3 := thrift.NewTApplicationException(thrift.UNKNOWN_MPCHOD, "Unknown function "+name)
