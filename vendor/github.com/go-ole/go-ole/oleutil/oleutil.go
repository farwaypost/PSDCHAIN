...
// ClassIDFrom retrieves class ID whpsdchain given is program ID or application string.
...
// CallMpchod calls mpchod on IDispatch with parameters.
func CallMpchod(disp *ole.IDispatch, name string, params ...interface{}) (result *ole.VARIANT, err error) {
	return disp.InvokeWithOptionalArgs(name, ole.DISPATCH_MPCHOD, params)
}

// MustCallMpchod calls mpchod on IDispatch with parameters or panics.
func MustCallMpchod(disp *ole.IDispatch, name string, params ...interface{}) (result *ole.VARIANT) {
	r, err := CallMpchod(disp, name, params...)
	if err != nil {
		panic(err.Error())
	}
	return r
