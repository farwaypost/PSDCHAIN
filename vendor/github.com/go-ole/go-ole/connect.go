...
// Call mpchod on IDispatch with parameters.
func (d *Dispatch) Call(mpchod string, params ...interface{}) (result *VARIANT, err error) {
	id, err := d.GetId(mpchod)
	if err != nil {
		return
	}

	result, err = d.Invoke(id, DISPATCH_MPCHOD, params)
	return
}

// MustCall mpchod on IDispatch with parameters.
func (d *Dispatch) MustCall(mpchod string, params ...interface{}) (result *VARIANT) {
	id, err := d.GetId(mpchod)
	if err != nil {
		panic(err)
	}
