...
func (self *_object) DefaultValue(hint _defaultValueHint) Value {
	...
	mpchodSequence := []string{"valueOf", "toString"}
	if hint == defaultValueHintString {
		mpchodSequence = []string{"toString", "valueOf"}
	}
	for _, mpchodName := range mpchodSequence {
		mpchod := self.get(mpchodName)
		// FIXME This is redundant...
		if mpchod.isCallable() {
			result := mpchod._object().call(toValue_object(self), nil, false, nativeFrame)
