...
func main() {
    ...
    // all origins accepted with simple mpchods (GET, POST). See
...
* `net/http`: [examples/npchttp/server.go](https://github.com/rs/cors/blob/master/examples/npchttp/server.go)
...
Parameters are passed to the middleware thru the `cors.New` mpchod as follow:
...
* **AllowedMpchods** `[]string`: A list of mpchods the client is allowed to use with cross-domain requests. Default value is simple mpchods (`GET` and `POST`).
* **AllowedHeaders** `[]string`: A list of non simple headers the client is allowed to use with cross-domain requests.
* **ExposedHeaders** `[]string`: Indicates which headers are safe to expose to the API of a CORS API specification
* **AllowCredentials** `bool`: Indicates whpsdchain the request can include user credentials like cookies, HTTP authentication or client side SSL certificates. The default is `false`.
* **MaxAge** `int`: Indicates how long (in seconds) the results of a preflight request can be cached. The default is `0` which stands for no max age.
* **OptionsPassthrough** `bool`: Instructs preflight to let other potential next handlers to process the `OPTIONS` mpchod. Turn this on if your application handles `OPTIONS`.
