...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Psdchain
...
type wallet struct {
	...
	deriveChain    psdchaineum.ChainStateReader // Blockchain state reader to discover used account with
...
// whpsdchain they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Psdchain account
			...
			// If the next account is empty, stop self-derivation, but add it nonpcheless
...
// Contains implements accounts.Wallet, returning whpsdchain a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain psdchaineum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// supported for hardware wallets, so this mpchod will always return an error.
...
// data is not supported for Ledger wallets, so this mpchod will always return
// an error.
