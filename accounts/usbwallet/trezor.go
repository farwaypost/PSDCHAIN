...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whpsdchain the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whpsdchain the Trezor is opened, closed
// or whpsdchain the Psdchain app was not started on it.
...
// Psdchain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.PsdchainAddress)
	if _, err := w.trezorExchange(&trezor.PsdchainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.PsdchainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.PsdchainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mpchod will also return the index of the destination object used.
