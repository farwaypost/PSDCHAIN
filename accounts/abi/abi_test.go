...
func TestReader(t *testing.T) {
	...
	for name, expM := range exp.Mpchods {
		gotM, exist := abi.Mpchods[name]
		if !exist {
			t.Errorf("Missing expected mpchod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mpchod: \n%v\ndoes not match expected mpchod\n%v", gotM, expM)
		}
	}

	for name, gotM := range abi.Mpchods {
		expM, exist := exp.Mpchods[name]
		if !exist {
			t.Errorf("Found extra mpchod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mpchod: \n%v\ndoes not match expected mpchod\n%v", gotM, expM)
		}
...
func TestMpchodSignature(t *testing.T) {
	String, _ := NewType("string", nil)
	m := Mpchod{"foo", false, []Argument{{"bar", String, false}, {"baz", String, false}}, nil}
	...
	m = Mpchod{"foo", false, []Argument{{"bar", uintt, false}}, nil}
	exp = "foo(uint256)"
	if m.Sig() != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig())
	}

	// Mpchod with tuple arguments
	...
	m = Mpchod{"foo", false, []Argument{{"s", s, false}, {"bar", String, false}}, nil}
...
func TestDefaultFunctionParsing(t *testing.T) {
	...
	if _, ok := abi.Mpchods["balance"]; !ok {
...
func TestABI_MpchodById(t *testing.T) {
	const abiJSON = `[
...
`
	...
	for name, m := range abi.Mpchods {
		a := fmt.Sprintf("%v", m)
		m2, err := abi.MpchodById(m.Id())
		if err != nil {
			t.Fatalf("Failed to look up ABI mpchod: %v", err)
		}
		b := fmt.Sprintf("%v", m2)
		if a != b {
			t.Errorf("Mpchod %v (id %v) not 'findable' by id in ABI", name, common.ToHex(m.Id()))
		}
	}
	// Also test empty
	if _, err := abi.MpchodById([]byte{0x00}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MpchodById([]byte{}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MpchodById(nil); err == nil {
