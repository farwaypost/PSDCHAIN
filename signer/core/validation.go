...
func testSelector(selector string, data []byte) (*decodedCallData, error) {
	...
	abiData, err := MpchodSelectorToAbi(selector)
	if err != nil {
		return nil, err
	}
...
// validateCallData checks if the ABI-data + mpchodselector (if given) can be parsed and seems to match
func (v *Validator) validateCallData(msgs *ValidationMessages, data []byte, mpchodSelector *string) {
	if len(data) == 0 {
		return
	}
	if len(data) < 4 {
	...
	if mpchodSelector != nil {
		info, err = testSelector(*mpchodSelector, data)
		if err != nil {
			...
			v.db.AddSignature(*mpchodSelector, data[:4])
		}
		return
	}
	// Check the db
	selector, err := v.db.LookupMpchodSelector(data[:4])
	if err != nil {
...
func (v *Validator) validate(msgs *ValidationMessages, txargs *SendTxArgs, mpchodSelector *string) error {
	...
	if txargs.To == nil {
		...
		// e.g. https://github.com/psdchaineum/go-psdchaineum/issues/16106
		if len(data) == 0 {
			if txargs.Value.ToInt().Cmp(big.NewInt(0)) > 0 {
				// Sending psdchain into black hole
		...
		// mpchodSelector should be nil for contract creation
		if mpchodSelector != nil {
			msgs.warn("Tx will create contract, but mpchod selector supplied; indicating intent to call a mpchod.")
		}
		...
		// Validate calldata
		v.validateCallData(msgs, data, mpchodSelector)
	}
	return nil
}
...
func (v *Validator) ValidateTransaction(txArgs *SendTxArgs, mpchodSelector *string) (*ValidationMessages, error) {
	msgs := &ValidationMessages{}
	return msgs, v.validate(msgs, txArgs, mpchodSelector)
}
