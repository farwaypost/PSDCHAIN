...
func (ui *HeadlessUI) OnApprovedTx(tx pchapi.SignTransactionResult) {
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "pch-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *pchapi.SignTransactionResult
		err       error
	)
	...
	control <- "Y"
	control <- "wrongpassword"
	res, err = api.SignTransaction(context.Background(), tx, &mpchodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &mpchodSig)
	if err != nil {
		t.Fatal(err)
	}
