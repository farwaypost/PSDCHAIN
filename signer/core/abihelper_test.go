...
func verify(t *testing.T, jsondata, calldata string, exp []interface{}) {
	...
	data, err := mpchod.Inputs.UnpackValues(argdata)
	if err != nil {
		t.Fatal(err)
	}
	if len(data) != len(exp) {
...
func TestNewUnpacker(t *testing.T) {
	...
	testcases := []unpackTest{
		...
		}, { // https://github.com/psdchaineum/wiki/wiki/Psdchain-Contract-ABI#examples
...
[
	...
	for i, hexdata := range []string{
		...
		// From https://github.com/psdchaineum/wiki/wiki/Psdchain-Contract-ABI
...
func TestSelectorUnmarshalling(t *testing.T) {
	...
	for id, selector := range db.db {
		...
		abistring, err = MpchodSelectorToAbi(selector)
		if err != nil {
			t.Error(err)
			return
		}
		...
		m, err := abistruct.MpchodById(common.Hex2Bytes(id[2:]))
		if err != nil {
			t.Error(err)
			return
		}
...
func TestCustomABI(t *testing.T) {
	...
	_, err = abidb.LookupMpchodSelector(calldata)
	if err == nil {
	...
	_, err = abidb2.LookupMpchodSelector(calldata)
	if err != nil {
