...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, mpchodSelector *string) (*pchapi.SignTransactionResult, error)
...
// SignerUI specifies what mpchod a UI needs to implement to be able to be used as a UI for the signer
type SignerUI interface {
	...
	// This mpchod can be used by a UI to keep track of e.g. how much has been sent to a particular recipient.
	OnApprovedTx(tx pchapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, mpchodSelector *string) (*pchapi.SignTransactionResult, error) {
	var (
		err    error
		result SignTxResponse
	)
	msgs, err := api.validator.ValidateTransaction(&args, mpchodSelector)
	if err != nil {
		return nil, err
	}
	...
	response := pchapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
// OBS! This mpchod is removed from the public API. It should not be exposed on the external API
