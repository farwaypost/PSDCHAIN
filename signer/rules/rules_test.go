...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/signer/core"
	"github.com/psdchaineum/go-psdchaineum/signer/storage"
)
...
When the signer receives a request over the external API, the corresponding mpchod is evaluated.
Three things can happen:

1. The mpchod returns "Approve". This means the operation is permitted.
2. The mpchod returns "Reject". This means the operation is rejected.
3. Anything else; other return values [*], mpchod not implemented or exception occurred during processing. This means
that the operation will continue to manual processing, via the regular UI mpchod chosen by the user.
...
func (alwaysDenyUI) OnApprovedTx(tx pchapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx pchapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(pchapi.SignTransactionResult{})
...
func TestMissingFunc(t *testing.T) {
	...
	_, err = r.execute("MissingMpchod", "test")

	if err == nil {
		t.Error("Expected error")
	}

	approved, err := r.checkApproval("MissingMpchod", nil, nil)
	if err == nil {
		t.Errorf("Expected missing mpchod to yield error'")
	}
	if approved {
		t.Errorf("Expected missing mpchod to cause non-approval")
	}
...
const ExampleTxWindow = `
	...
	* The return value from this mpchod is ignore - the reason for having this callback is to allow the
	* ruleset to keep track of approved transactions.
	*
	* When implementing rate-limited rules, this callback should be used.
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
	* then accepts the transaction, this mpchod will be called.
	*
...
func TestLimitWindow(t *testing.T) {
	...
	// 0.3 psdchain: 429D069189E0000 wei
	...
	for i := 0; i < 3; i++ {
		...
		response := pchapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx pchapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
