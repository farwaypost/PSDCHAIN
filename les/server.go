...
func NewLesServer(pch *pch.Psdchain, config *pch.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(pch.BlockChain().Config(), light.DefaultServerIndexerConfig, false, config.NetworkId, pch.EventMux(), pch.Engine(), newPeerSet(), pch.BlockChain(), pch.TxPool(), pch.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(pch.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		lesCommons: lesCommons{
			...
			chtIndexer:       light.NewChtIndexer(pch.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(pch.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		quitSync:  quitSync,
		lesTopics: lesTopics,
	...
	srv.chtIndexer.Start(pch.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(pch.ChainDb())
	return srv, nil
}
...
func newCostStats(db pchdb.Database) *requestCostStats {
