...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Psdchain
...
func (pch *LightPsdchain) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-pch.shutdownChan:
					return

				case request := <-pch.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					compVectors, err := light.GetBloomBits(task.Context, pch.odr, task.Bit, task.Sections)
					if err == nil {
