...
func newTester(t *testing.T, confOverride func(*pch.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Psdchain service within
	...
	pchConf := &pch.Config{
		...
		Psdchainbase: common.HexToAddress(testAddress),
		Ethash: pchash.Config{
			PowMode: pchash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(pchConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return pch.New(ctx, pchConf) }); err != nil {
		t.Fatalf("failed to register Psdchain protocol: %v", err)
	}
	...
	stack.Service(&psdchaineum)

	return &tester{
		workspace: workspace,
		stack:     stack,
		psdchaineum:  psdchaineum,
		console:   console,
		input:     prompter,
		output:    printer,
	}
