...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/accounts/usbwallet"
	"github.com/psdchaineum/go-psdchaineum/log"
	"github.com/psdchaineum/go-psdchaineum/rpc"
	"github.com/robertkrimen/otto"
)

// bridge is a collection of JavaScript utility mpchods to bride the .js runtime
// environment and the Go RPC connection backing the remote mpchod calls.
type bridge struct {
...
// NewAccount is a wrapper around the personal.newAccount RPC mpchod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC mpchod (saved in jpch.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	...
	ret, err := call.Otto.Call("jpch.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jpch.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
	...
	if val, err = call.Otto.Call("jpch.openWallet", nil, wallet, passwd); err != nil {
		throwJSException(err.Error())
	}
	return val
}

// UnlockAccount is a wrapper around the personal.unlockAccount RPC mpchod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC mpchod (saved in jpch.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jpch.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// Sign is a wrapper around the personal.sign RPC mpchod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC mpchod (saved in
// jpch.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jpch.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("pch.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
...
// Send implements the web3 provider "send" mpchod.
func (b *bridge) Send(call otto.FunctionCall) (response otto.Value) {
	...
	for _, req := range reqs {
		...
		err = b.client.Call(&result, req.Mpchod, req.Params...)
		switch err := err.(type) {
		case nil:
			if result == nil {
