...
func main() {
	...
	// Pre-generate the pchash mining DAG so we don't race
	pchash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".pchash"))
	...
	for _, node := range nodes {
		...
		if err := node.Service(&psdchaineum); err != nil {
			panic(err)
		}
		if err := psdchaineum.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var psdchaineum *pch.Psdchain
		if err := nodes[index%len(nodes)].Service(&psdchaineum); err != nil {
			panic(err)
		}
		...
		if err := psdchaineum.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := psdchaineum.TxPool().Stats(); pend > 2048 {
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the Psdchain node
	...
	// Start the node and configure a full Psdchain node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, err
	}
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return pch.New(ctx, &pch.Config{
			Genesis:         genesis,
			...
			GPO:             pch.DefaultConfig.GPO,
			Ethash:          pch.DefaultConfig.Ethash,
