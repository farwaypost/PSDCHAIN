...
// Package miner implements Psdchain block creation and mining.
package miner
...
// Backend wraps all mpchods required for mining.
...
type Miner struct {
	...
	canStart    int32 // can start indicates whpsdchain we can start the mining operation
	shouldStart int32 // should start indicates whpsdchain we should start after sync
}

func New(pch Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, recommit time.Duration, gasFloor, gasCeil uint64, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		pch:      pch,
		mux:      mux,
		engine:   engine,
		exitCh:   make(chan struct{}),
		worker:   newWorker(config, engine, pch, mux, recommit, gasFloor, gasCeil, isLocalBlock),
		canStart: 1,
	}
	go miner.update()
...
// change between multiple mpchod calls
func (self *Miner) PendingBlock() *types.Block {
	return self.worker.pendingBlock()
}

func (self *Miner) SetPsdchainbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setPsdchainbase(addr)
}
