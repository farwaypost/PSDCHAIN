...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/common/bitutil"
...
func GetChtRoot(db pchdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db pchdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    pchdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db pchdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := pchdb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, pchdb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func GetBloomTrieRoot(db pchdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db pchdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db pchdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := pchdb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, pchdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
