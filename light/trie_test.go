...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb  = pchdb.NewMemDatabase()
		lightdb = pchdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, pchash.NewFullFaker(), vm.Config{}, nil)
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, pchash.NewFaker(), fulldb, 4, testChainGen)
