...
creating a collection of devp2p nodes, connecting them togpsdchain to form a
...
if a node is created without an explicit service), exposes mpchods for
...
connecting two nodes togpsdchain).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of sompching happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` mpchod can be called
with a `Step` which has the following fields:

* `Action` - a function which performs some action in the network

* `Expect` - an expectation function which returns whpsdchain or not a
...
As a concrete example, consider a simulated network of Psdchain nodes. An
...
On return, the `Simulation.Run` mpchod returns a `StepResult` which can be used
...
p2psim node rpc <node> <mpchod> [<args>] [--subscribe]
```
