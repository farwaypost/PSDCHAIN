...
// starting nodes and connecting them togpsdchain.
//
...
// Connect connects two nodes togpsdchain by calling the "admin_addPeer" RPC
// mpchod on the "one" node so that it connects to the "other" node
...
// mpchod on the "one" node so that it disconnects from the "other" node
...
// NOTE:
// it also checks whpsdchain there has been recent attempt to connect the peers
...
type Node struct {
	...
	// up tracks whpsdchain or not the node is running
	up   bool
	upMu sync.RWMutex
}
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/psdchainsphere/go-psdchaineum/issues/1177
	node := struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}{}
...
type Conn struct {
	...
	// Up tracks whpsdchain or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whpsdchain both nodes are currently up
