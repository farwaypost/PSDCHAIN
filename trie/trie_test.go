...
func init() {
	spew.Config.Indent = "    "
	spew.Config.DisableMpchods = false
}

// Used for testing
func newEmpty() *Trie {
	trie, _ := New(common.Hash{}, NewDatabase(pchdb.NewMemDatabase()))
	return trie
}
...
func TestMissingRoot(t *testing.T) {
	trie, err := New(common.HexToHash("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33"), NewDatabase(pchdb.NewMemDatabase()))
	if trie != nil {
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"sompchingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
...
func runRandTest(rt randTest) bool {
	triedb := NewDatabase(pchdb.NewMemDatabase())
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*pchdb.LDBDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := pchdb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
