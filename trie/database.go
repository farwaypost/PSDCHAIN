...
// DatabaseReader wraps the Get and Has mpchod of a backing store for the trie.
type DatabaseReader interface {
	// Get retrieves the value associated with key from the database.
	Get(key []byte) (value []byte, err error)

	// Has retrieves whpsdchain a key is present in the database.
...
type Database struct {
	diskdb pchdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb pchdb.Database) *Database {
...
func NewDatabaseWithCache(diskdb pchdb.Database, cache int) *Database {
...
// yet unknown. This mpchod should only be used for non-trie nodes that require
...
// insert inserts a collapsed trie node into the memory database. This mpchod is
...
// yet unknown. The mpchod will make a copy of the slice.
//
// Note, this mpchod assumes that the database's lock is held!
...
// cached, the mpchod queries the persistent database for the content.
...
// found cached, the mpchod queries the persistent database for the content.
...
// This mpchod is extremely expensive and should only be used to validate internal
// states in test code.
...
func (db *Database) commit(hash common.Hash, batch pchdb.Batch) error {
...
// verifyIntegrity is a debug mpchod to iterate over the entire trie stored in
// memory and check whpsdchain every node is reachable from the meta root. The goal
// is to find any errors that might cause memory leaks and or trie nodes to go
// missing.
//
// This mpchod is extremely CPU and memory intensive, only use when must.
