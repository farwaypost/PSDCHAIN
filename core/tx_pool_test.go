...
func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(pchdb.NewMemDatabase()))
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	stdb := c.statedb
	if *c.trigger {
		c.statedb, _ = state.New(common.Hash{}, state.NewDatabase(pchdb.NewMemDatabase()))
		// simulate that the new head block included tx0 and tx1
		c.statedb.SetNonce(c.address, 2)
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Psdchain))
		*c.trigger = false
	}
	return stdb, nil
}
...
// state reset and tests whpsdchain the pending state is in sync with the
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Psdchain))
...
// Tests that the transaction limits are enforced the same way irrelevant whpsdchain
