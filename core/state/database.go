...
// Trie is a Psdchain Merkle Trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb pchdb.Putter) error
}
...
func NewDatabase(db pchdb.Database) Database {
...
func NewDatabaseWithCache(db pchdb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb pchdb.Putter) error {
