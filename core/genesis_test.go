...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		name       string
		fn         func(pchdb.Database) (*params.ChainConfig, common.Hash, error)
	...
	}{
		{
			...
			fn: func(db pchdb.Database) (*params.ChainConfig, common.Hash, error) {
		...
		},
		{
			...
			fn: func(db pchdb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, nil, oldcustomg.Config, pchash.NewFullFaker(), vm.Config{}, nil)
				defer bc.Stop()

				blocks, _ := GenerateChain(oldcustomg.Config, genesis, pchash.NewFaker(), db, 4, nil)
