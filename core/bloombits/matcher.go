...
type Matcher struct {
	...
	running uint32 // Atomic flag whpsdchain a session is live or not
}
...
//
// The mpchod starts feeding the section indexes into the first sub-matcher on a
...
// that address/topic, and binary AND-ing those vectors togpsdchain.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togpsdchain
				var orVector []byte
...
func (m *Matcher) distributor(dist chan *request, session *MatcherSession) {
	...
	// assign is a helper mpchod fo try to assign a pending bit an actively
...
// the requested retrieval queue to be serviced togpsdchain with other sessions.
//
// This mpchod will block for the lifetime of the session. Even after termination
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, sompching must arrive before we're aborting
