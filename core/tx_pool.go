...
type TxPoolConfig struct {
	...
	NoLocals  bool             // Whpsdchain local transaction handling should be disabled
...
// manner. This mpchod is only ever used in the tester!
...
// validateTx checks whpsdchain a transaction is valid according to the consensus
...
//
// Note, this mpchod assumes the pool lock is held!
...
// and returns whpsdchain it was inserted or an older was better.
//
// Note, this mpchod assumes the pool lock is held!
...
func (pool *TxPool) addTxsLocked(txs []*types.Transaction, local bool) []error {
	...
	// Only reprocess the internal state if sompching was actually added
	if len(dirty) > 0 {
...
// cannot be derived, this mpchod returns false.
