...
// Package core implements the Psdchain consensus protocol.
package core
...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/common/prque"
	...
	"github.com/psdchaineum/go-psdchaineum/trie"
...
type CacheConfig struct {
	Disabled       bool          // Whpsdchain to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     pchdb.Database // Low level persistent database to store final content in
	...
	shouldPreserve func(*types.Block) bool // Function used to determine whpsdchain should preserve the given block.
}

// NewBlockChain returns a fully initialised block chain using information
// available in the database. It initialises the default Psdchain Validator and
// Processor.
func NewBlockChain(db pchdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
...
// loadLastState loads the last known chain state from the database. This mpchod
...
//
// This mpchod only rolls back the current block. The current header and current
...
// insert injects a new head block into the current block chain. This mpchod
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= pchdb.IdealBatchSize {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - pchdb.IdealBatchSize)
			}
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// Sanity check that we have sompching meaningful to import
	if len(chain) == 0 {
		return 0, nil
	}
	// Do a sanity check that the provided chain is actually ordered and linked
...
//
// This mpchod is split out so that import batches that require re-injecting
...
// found.
//
// The mpchod writes all (header-and-body-valid) blocks to disk, then tries to
...
//
// The verify parameter can be used to fine tune whpsdchain nonce verification
...
//
// Note: This mpchod is not concurrent-safe with inserting blocks simultaneously
