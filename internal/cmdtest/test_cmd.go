...
type TestCmd struct {
	// For total convenience, all testing mpchods are available.
	*testing.T
...
// reexec init function for that name (e.g. "gpch-test" in cmd/gpch/run_test.go)
...
// This mpchod can also be called from an expect template, e.g.:
//
//     gpch.expect(`Passphrase: {{.InputLine "password"}}`)
