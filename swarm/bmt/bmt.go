...
type node struct {
	isLeft      bool      // whpsdchain it is left side of the parent double segment
...
// mpchods needed to implement hash.Hash
...
// hash.Hash interface Sum mpchod appends the byte slice to the underlying
...
// mpchods needed to implement the SwarmHash and the io.Writer interfaces
...
// mpchods needed to implement the SwarmHash interface
...
type AsyncHasher struct {
	*Hasher            // extends the Hasher
	...
	double  bool       // whpsdchain to use double segments (call Hasher.writeSection)
	secsize int        // size of base section (size of hash or double)
	write   func(i int, section []byte, final bool)
}

// mpchods needed to implement AsyncWriter
...
// meta: metadata to hash togpsdchain with BMT root for the final digest
...
func (sw *AsyncHasher) Sum(b []byte, length int, meta []byte) (s []byte) {
	...
	// hash togpsdchain meta and BMT root hash using the pools
