...
#
# The cluster contains a bootnode, a gpch node and multiple swarm nodes, with
...
# static gpch configuration
GPCH_IP="192.168.33.3"
GPCH_RPC_PORT="8545"
GPCH_RPC_URL="http://${GPCH_IP}:${GPCH_RPC_PORT}"
...
# network namespaces togpsdchain
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the gpch and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gpch node with --datadir pointing at <base-dir>/gpch
# and a single, unlocked account with password "gpch"
start_geth_node() {
  local dir="${base_dir}/gpch"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gpch account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GPCH_IP}"
    --rpcport   "${GPCH_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gpch" "${GPCH_IP}" "$(which gpch)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vpch pair
  local vpch0="vpch${name}0"
  local vpch1="vpch${name}1"
  ip link add name "${vpch0}" type vpch peer name "${vpch1}"

  # add one end to the bridge
  ip link set dev "${vpch0}" master "${BRIDGE_NAME}"
  ip link set dev "${vpch0}" up

  # add the other end to the namespace, rename it pch0 and give it the ip
  ip link set dev "${vpch1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vpch1}" name "pch0"
  ip netns exec "${name}" ip link set dev "pch0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "pch0"
}
...
create_account() {
  ...
  gpch --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
