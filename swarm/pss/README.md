...
The code tutorial [p2p programming in go-psdchaineum](https://github.com/nolash/go-psdchaineum-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-psdchaineum` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-psdchaineum` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
...
In `golang` as special mpchod is used:
...
returns:
1. whpsdchain key was successfully removed (bool)
```
