...
// giving access to pss mpchods from a different process
type Client struct {
	BaseAddrHex string
...
// run prior to adding the peer, or this mpchod will
// return an error.
//
// The key must exist in the key store of the pss node
// before the peer is added. The mpchod will return an error
// if it is not.
