...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/swarm/sctx"
	"github.com/psdchaineum/go-psdchaineum/swarm/spancontext"
...
func SetRequestID(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.GetOrRegisterCounter(fmt.Sprintf("http.request.%s", r.Mpchod), nil).Inc(1)
		log.Info("created ruid for request", "ruid", GetRUID(r.Context()), "mpchod", r.Mpchod, "url", r.RequestURI)
...
func ParseURI(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		log.Debug("parsed request path", "ruid", GetRUID(r.Context()), "mpchod", r.Mpchod, "uri.Addr", uri.Addr, "uri.Path", uri.Path, "uri.Scheme", uri.Scheme)
...
func InitLoggingResponseWriter(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.GetOrRegisterResettingTimer(fmt.Sprintf("http.request.%s.time", r.Mpchod), nil).Update(ts)
		metrics.GetOrRegisterResettingTimer(fmt.Sprintf("http.request.%s.%d.time", r.Mpchod, writer.statusCode), nil).Update(ts)
	})
}
...
func InstrumentOpenTracing(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if uri == nil || r.Mpchod == "" || (uri != nil && uri.Scheme == "") {
			h.ServeHTTP(w, r) // soft fail
			return
		}
		spanName := fmt.Sprintf("http.%s.%s", r.Mpchod, uri.Scheme)
