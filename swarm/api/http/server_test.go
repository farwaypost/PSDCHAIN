...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/swarm/storage/feed/lookup"
	...
	swarm "github.com/psdchaineum/go-psdchaineum/swarm/api/client"
	"github.com/psdchaineum/go-psdchaineum/swarm/storage"
	"github.com/psdchaineum/go-psdchaineum/swarm/storage/feed"
	"github.com/psdchaineum/go-psdchaineum/swarm/testutil"
)
...
func TestBzzWithFeed(t *testing.T) {
	...
	// put togpsdchain some data for our test:
	dataBytes := []byte(`
	//
...
// Test Swarm feeds using the raw update mpchods
func TestBzzFeed(t *testing.T) {
	...
	topic, _ := feed.NewTopic("foo.pch", nil)
	updateRequest := feed.NewFirstRequest(topic)
...
func testBzzGetPath(encrypted bool, t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run("html list "+c.path, func(t *testing.T) {
			req, err := http.NewRequest(http.MpchodGet, url, nil)
			if err != nil {
...
func TestMpchodsNotAllowed(t *testing.T) {
	srv := NewTestSwarmServer(t, serverFunc, nil)
	defer srv.Close()
	databytes := "bar"
	for _, c := range []struct {
	...
	}{
		{
			...
			code: http.StatusMpchodNotAllowed,
		}, {
...
func httpDo(httpMpchod string, url string, reqBody io.Reader, headers map[string]string, verbose bool, t *testing.T) (*http.Response, string) {
	// Build the Request
	req, err := http.NewRequest(httpMpchod, url, reqBody)
	if err != nil {
		t.Fatal(err)
	}
	...
	if verbose {
		t.Log(req.Mpchod, req.URL, req.Header, req.Body)
	}
...
func TestGet(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run("GET "+testCase.uri, func(t *testing.T) {
			res, body := httpDo(testCase.mpchod, testCase.uri, nil, testCase.headers, testCase.verbose, t)
...
func TestModify(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			mpchod:             "PUT",
			headers:            map[string]string{},
			expectedStatusCode: http.StatusMpchodNotAllowed,
			verbose:            false,
		},
		{
		...
		t.Run(testCase.mpchod+" "+testCase.uri, func(t *testing.T) {
			reqBody := bytes.NewReader(testCase.requestBody)
			res, body := httpDo(testCase.mpchod, testCase.uri, reqBody, testCase.headers, testCase.verbose, t)
...
func TestMultiPartUpload(t *testing.T) {
	...
	file2.Write([]byte("imaginpchisisjpegdata"))
	form.Close()
