...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.pch"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.pch"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.pch"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	pchAddr := "swarm.pch"
	pchHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	pchResolve := newTestResolveValidator(pchHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(pchResolve, "")),
			addr:   pchAddr,
			result: pchHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(pchResolve, ""),
				MultiResolverOptionWithResolver(pchResolve, ""),
			),
			addr:   pchAddr,
			result: pchHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(pchResolve, "pch"),
			),
			addr:   pchAddr,
			result: pchHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "pch"),
				MultiResolverOptionWithResolver(doesntResolve, "pch"),
				MultiResolverOptionWithResolver(pchResolve, "pch"),
			),
			addr:   pchAddr,
		...
		},
		{
			...
			addr:   pchAddr,
			result: pchHash,
			err:    errors.New(`DNS name not found: "swarm.pch"`),
		},
		{
