...
import (
	...
	swarmhttp "github.com/psdchaineum/go-psdchaineum/swarm/api/http"
...
func TestClientBzzWithFeed(t *testing.T) {
	...
	if len(manifestAddress) != storage.AddressLength {
		t.Fatalf("Sompching went wrong. Got a hash of an unexpected length. Expected %d bytes. Got %d", storage.AddressLength, len(manifestAddress))
	}
