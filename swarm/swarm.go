...
import (
	...
	"github.com/psdchaineum/go-psdchaineum/contracts/chequebook"
	"github.com/psdchaineum/go-psdchaineum/contracts/ens"
	...
	"github.com/psdchaineum/go-psdchaineum/p2p/protocols"
	"github.com/psdchaineum/go-psdchaineum/params"
	"github.com/psdchaineum/go-psdchaineum/rpc"
	"github.com/psdchaineum/go-psdchaineum/swarm/api"
	httpapi "github.com/psdchaineum/go-psdchaineum/swarm/api/http"
	"github.com/psdchaineum/go-psdchaineum/swarm/fuse"
	"github.com/psdchaineum/go-psdchaineum/swarm/log"
	"github.com/psdchaineum/go-psdchaineum/swarm/network"
	"github.com/psdchaineum/go-psdchaineum/swarm/network/stream"
	"github.com/psdchaineum/go-psdchaineum/swarm/pss"
	"github.com/psdchaineum/go-psdchaineum/swarm/state"
	"github.com/psdchaineum/go-psdchaineum/swarm/storage"
	"github.com/psdchaineum/go-psdchaineum/swarm/storage/feed"
	"github.com/psdchaineum/go-psdchaineum/swarm/storage/mock"
	"github.com/psdchaineum/go-psdchaineum/swarm/swap"
	"github.com/psdchaineum/go-psdchaineum/swarm/tracing"
)
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = pchclient.Dial(config.SwapAPI)
		if err != nil {
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	pchClient := pchclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, pchClient)
	if err != nil {
		return nil, err
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := pchclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
