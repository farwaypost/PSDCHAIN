...
import (
	...
	gpchmetrics "github.com/psdchaineum/go-psdchaineum/metrics"
...
func Setup(ctx *cli.Context) {
	if gpchmetrics.Enabled {
		...
		go gpchmetrics.CollectProcessMetrics(2 * time.Second)
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gpchmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", tagsMap)
		}

		if enableAccountingExport {
			log.Info("Exporting swarm accounting metrics to InfluxDB")
			go influxdb.InfluxDBWithTags(gpchmetrics.AccountingRegistry, 10*time.Second, endpoint, database, username, password, "accounting.", tagsMap)
		}
	}
}
