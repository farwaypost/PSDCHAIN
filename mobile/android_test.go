...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gpch.LightScryptN, Gpch.LightScryptP);
		...
		try {
			...
			// data from this mpchod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.psdchaineum", new NodeConfig());
			node.start();
			...
			PsdchainClient ec = node.getPsdchainClient();
	...
	// signatures too. Regression test for go-psdchaineum issue #14599.
...
//
// This mpchod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogpsdchain
	...
	workspace, err := ioutil.TempDir("", "gpch-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/psdchaineum/gpchtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gpch and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.psdchaineum", "github.com/psdchaineum/go-psdchaineum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gpch.aar"), "gpch.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "psdchaineum", "gpchtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.psdchaineum.gpchtest"
