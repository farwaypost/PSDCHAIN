...
func TestClientBatchRequest(t *testing.T) {
	...
	wantResult := []BatchElem{
		...
		},
		{
			...
			Args:   []interface{}{1, 2, 3},
			Result: new(int),
			Error:  &jsonError{Code: -32601, Message: "The mpchod no_such_mpchod_ does not exist/is not available"},
		},
	}
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("pch", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("pch", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
// This test reproduces https://github.com/psdchaineum/go-psdchaineum/issues/17837 where the
...
func TestClientReconnect(t *testing.T) {
	...
	// We spawn multiple calls here to check whpsdchain this hangs somehow.
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-psdchaineum-test-ipc-%d-%d", os.Getpid(), rand.Int63())
