...
func TestJSONRequestParsing(t *testing.T) {
	...
	req := bytes.NewBufferString(`{"id": 1234, "jsonrpc": "2.0", "mpchod": "calc_add", "params": [11, 22]}`)
	var str string
	...
	if requests[0].mpchod != "add" {
		t.Fatalf("Expected mpchod 'Add' but got '%s'", requests[0].mpchod)
	}
