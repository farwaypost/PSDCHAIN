...
type mpchodNotFoundError struct {
	service string
	mpchod  string
}

func (e *mpchodNotFoundError) ErrorCode() int { return -32601 }

func (e *mpchodNotFoundError) Error() string {
	return fmt.Sprintf("The mpchod %s%s%s does not exist/is not available", e.service, serviceMpchodSeparator, e.mpchod)
}
