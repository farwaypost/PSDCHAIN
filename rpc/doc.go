...
/*
Package rpc provides access to the exported mpchods of an object across a network
or other I/O connection. After creating a server instance objects can be registered,
making it visible from the outside. Exported mpchods that follow specific
conventions can be called remotely. It also has support for the publish/subscribe
pattern.

Mpchods that satisfy the following criteria are made available for remote access:
 - object must be exported
 - mpchod must be exported
 - mpchod returns 0, 1 (response or error) or 2 (response and error) values
 - mpchod argument(s) must be exported or builtin types
 - mpchod returned value(s) must be exported or builtin types
...
This RPC mpchod can be called with 2 integers and a null value as third argument.
...
arguments. It will pass the mod argument as nil to the RPC mpchod.

The server offers the ServeCodec mpchod which accepts a ServerCodec instance. It will
...
A mpchod that is considered eligible for notifications must satisfy the following criteria:
 - object must be exported
 - mpchod must be exported
 - first mpchod argument type must be context.Context
 - mpchod argument(s) must be exported or builtin types
 - mpchod must return the tuple Subscription, error
