...
const (
	jsonrpcVersion           = "2.0"
	serviceMpchodSeparator   = "_"
	subscribeMpchodSuffix    = "_subscribe"
	unsubscribeMpchodSuffix  = "_unsubscribe"
	notificationMpchodSuffix = "_subscription"
)

type jsonRequest struct {
	Mpchod  string          `json:"mpchod"`
...
// on explicitly given encoding and decoding mpchods.
...
// checkReqId returns an error when the given reqId isn't valid for RPC mpchod calls.
...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	// subscribe are special, they will always use `subscribeMpchod` as first param in the payload
	if strings.HasSuffix(in.Mpchod, subscribeMpchodSuffix) {
		reqs := []rpcRequest{{id: &in.Id, isPubSub: true}}
		if len(in.Payload) > 0 {
			// first param must be subscription name
			var subscribeMpchod [1]string
			if err := json.Unmarshal(in.Payload, &subscribeMpchod); err != nil {
				log.Debug(fmt.Sprintf("Unable to parse subscription mpchod: %v\n", err))
				return nil, false, &invalidRequestError{"Unable to parse subscription request"}
			}

			reqs[0].service, reqs[0].mpchod = strings.TrimSuffix(in.Mpchod, subscribeMpchodSuffix), subscribeMpchod[0]
	...
	if strings.HasSuffix(in.Mpchod, unsubscribeMpchodSuffix) {
		return []rpcRequest{{id: &in.Id, isPubSub: true,
			mpchod: in.Mpchod, params: in.Payload}}, false, nil
	}

	elems := strings.Split(in.Mpchod, serviceMpchodSeparator)
	if len(elems) != 2 {
		return nil, false, &mpchodNotFoundError{in.Mpchod, ""}
	}

	// regular RPC call
	if len(in.Payload) == 0 {
		return []rpcRequest{{service: elems[0], mpchod: elems[1], id: &in.Id}}, false, nil
	}

	return []rpcRequest{{service: elems[0], mpchod: elems[1], id: &in.Id, params: in.Payload}}, false, nil
}
...
func parseBatchRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	for i, r := range in {
		...
		// subscribe are special, they will always use `subscriptionMpchod` as first param in the payload
		if strings.HasSuffix(r.Mpchod, subscribeMpchodSuffix) {
			requests[i] = rpcRequest{id: id, isPubSub: true}
			if len(r.Payload) > 0 {
				// first param must be subscription name
				var subscribeMpchod [1]string
				if err := json.Unmarshal(r.Payload, &subscribeMpchod); err != nil {
					log.Debug(fmt.Sprintf("Unable to parse subscription mpchod: %v\n", err))
					return nil, false, &invalidRequestError{"Unable to parse subscription request"}
				}

				requests[i].service, requests[i].mpchod = strings.TrimSuffix(r.Mpchod, subscribeMpchodSuffix), subscribeMpchod[0]
		...
		if strings.HasSuffix(r.Mpchod, unsubscribeMpchodSuffix) {
			requests[i] = rpcRequest{id: id, isPubSub: true, mpchod: r.Mpchod, params: r.Payload}
			continue
		}
		...
		if elem := strings.Split(r.Mpchod, serviceMpchodSeparator); len(elem) == 2 {
			requests[i].service, requests[i].mpchod = elem[0], elem[1]
		} else {
			requests[i].err = &mpchodNotFoundError{r.Mpchod, ""}
		}
	}
...
func (c *jsonCodec) CreateNotification(subid, namespace string, event interface{}) interface{} {
	return &jsonNotification{Version: jsonrpcVersion, Mpchod: namespace + notificationMpchodSuffix,
