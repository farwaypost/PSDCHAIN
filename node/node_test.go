...
// Tests whpsdchain services can be registered and duplicates caught.
...
func TestAPIGather(t *testing.T) {
	...
	makeAPI := func(result string) *OneMpchodAPI {
		return &OneMpchodAPI{fun: func() { calls <- result }}
	}
	services := map[string]struct {
		APIs  []rpc.API
		Maker InstrumentingWrapper
	...
	tests := []struct {
		...
		{"single_theOneMpchod", "single.v1"},
		{"multi_theOneMpchod", "multi.v1"},
		{"multi.v2_theOneMpchod", "multi.v2"},
		{"multi.v2.nested_theOneMpchod", "multi.v2.nested"},
	}
	for i, test := range tests {
		if err := client.Call(nil, test.Mpchod); err != nil {
