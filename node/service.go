...
// as well as utility mpchods to operate on the service environment.
...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (pchdb.Database, error) {
	if ctx.config.DataDir == "" {
		return pchdb.NewMemDatabase(), nil
	}
	db, err := pchdb.NewLDBDatabase(ctx.config.ResolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
