...
// Set of services all wrapping the base NoopService resulting in the same mpchod
...
// mpchods can be instrumented both return value as well as event hook wise.
...
// InstrumentingWrapper is a mpchod to specialize a service constructor returning
...
// same mpchod signatures but different outer types.
...
// OneMpchodAPI is a single-mpchod API handler to be returned by test services.
type OneMpchodAPI struct {
	fun func()
}

func (api *OneMpchodAPI) TheOneMpchod() {
	if api.fun != nil {
		api.fun()
	}
}
