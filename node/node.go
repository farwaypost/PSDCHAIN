...
import (
	...
	"github.com/prompcheus/prompcheus/util/flock"
)
...
func New(conf *Config) (*Node, error) {
	...
	// Ensure that the AccountManager mpchod works before the node has started.
	// We rely on this in cmd/gpch.
...
// startRPC is a helper mpchod to start all the various RPC endpoint during node
...
// at the time of invocation, the mpchod immediately returns.
func (n *Node) Wait() {
	n.lock.RLock()
	if n.server == nil {
		n.lock.RUnlock()
...
// Server retrieves the currently running P2P network layer. This mpchod is meant
...
func (n *Node) OpenDatabase(name string, cache, handles int) (pchdb.Database, error) {
	if n.config.DataDir == "" {
		return pchdb.NewMemDatabase(), nil
	}
	return pchdb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
