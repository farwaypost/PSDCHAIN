...
//
// The following mpchods are needed to implement a node.Service:
//  - Protocols() []p2p.Protocol - devp2p protocols the service can communicate on
//  - APIs() []rpc.API           - api mpchods the service wants to expose on rpc channels
//  - Start() error              - mpchod invoked when the node is ready to start the service
//  - Stop() error               - mpchod invoked when the node terminates the service
...
func ExampleService() {
	...
	// the factory mpchod approach is to support service restarts without relying on the
